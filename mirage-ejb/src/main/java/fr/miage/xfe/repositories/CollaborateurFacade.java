...
public class CollaborateurFacade extends AbstractFacade<Collaborateur> implements CollaborateurFacadeLocal {
    ...
    public List<Competence> listerCompCollaborateur(Collaborateur collaborateur) {
        return em.createQuery("SELECT c FROM Collaborateur c1, Competence c WHERE c1.candidat.competences.idcompetence IN (c.idcompetence) AND c1.idcollaborateur = :id").setParameter("id", collaborateur.getIdcollaborateur()).getResultList();
    }
}
